...
public class PlayerMovementNewInputSystem : MonoBehaviour
{
    ...
    public bool IsSpining { get => isSpining; set => isSpining = value; }

    private void Awake()
    {
        playerHealth = GetComponent<PlayerHealth>();
        groundDetection = transform.GetChild(3);
    ...
    private void Update()
    {
        ...
        animator.SetFloat(GameConstants.FLOATVELOCITY_KEY, rb.linearVelocity.y);
    }
    ...
    private void MovePlayer()
    {
        ...
        if (GeneralDetection(wallCheckDistance, Vector2.left, Color.green) || GeneralDetection(wallCheckDistance, Vector2.right, Color.green))
        {
            // Detiene el movimiento y la aceleración
            rb.linearVelocity = new Vector2(0f, rb.linearVelocity.y);
            AccelerationTimer = 0f;
            isSkidding = false;
            animator.SetBool(GameConstants.SKIDDINGBOOL_KEY, false);
        ...
        if (inputY < -0.7f && !(AccelerationTimer < 0.01f))
        {   
            isSpining = true;
            animator.SetBool(GameConstants.SPININGBOOL_KEY, true);
        } else if (AccelerationTimer < 0.01f)
        {
            isSpining = false;
            animator.SetBool(GameConstants.SPININGBOOL_KEY, false);
        }



        // --- ACELERATION ---
        if (Mathf.Abs(inputX) > 0.1f && !isSpining && !(inputY < -0.7))
        {
            ...
            {
                isSkidding = true;
                animator.SetBool(GameConstants.SKIDDINGBOOL_KEY, true);
                if (!animator.GetBool(GameConstants.JUMPINGBOOL_KEY))
                {
        ...
        // --- ANIMATION ---
        animator.SetFloat(GameConstants.FLOATSPEED_KEY, Mathf.Abs(rb.linearVelocity.x));

        if (currentSpeed < mediumMoveSpeed)
        {
            // Velocidad baja → animación a velocidad normal
            animator.speed = 1f;
        }
        else if (currentSpeed >= mediumMoveSpeed && currentSpeed < maxMoveSpeed)
        {
            // Velocidad media → animación un poco más rápida
            animator.speed = 1.8f;
        }
        else if (currentSpeed >= maxMoveSpeed)
        {
            // Velocidad máxima → animación al doble
            animator.speed = 2.3f;
        }

        if (IsGrounded() && !animator.GetBool(GameConstants.JUMPINGBOOL_KEY) && rb.linearVelocity.y > 0)
        {
    ...
    public void ApplyKnockback(Vector2 hitSourcePosition, bool isBoss)
    {
    ...
    private IEnumerator RecoverFromKnockback()
    {
        ...
        isKnockedBack = false;
        animator.ResetTrigger(GameConstants.TRIGGERHIT_KEY);
    }
    ...
    private void CalculateJump()
    {
        if (jumpInput && IsGrounded())
        {
            ...
            animator.SetBool(GameConstants.JUMPINGBOOL_KEY, true);
        }

        // Detects landing
        bool grounded = IsGrounded();
        if (grounded && !wasGrounded)
            animator.SetBool(GameConstants.JUMPINGBOOL_KEY, false);
